 ...
 * @author Huzzu
 */
public class Service_Report2 extends javax.swing.JFrame {
    ...
    private void fillByID (JTable table, String ValueToSearch)
    {
        ...
        ps=con.prepareStatement("Select * from Service where (Service_ID || Service_Category || Service_Name || Service_Code || Service_Cost || Description || hour || minute) like '%"+ValueToSearch+"%'");
        rs=ps.executeQuery();
        ...
        while(rs.next()){
            ...
            row[6]=rs.getString(8)+":"+rs.getString(9);
    ...
    private void fillByCategory (JTable table, String ValueToSearch)
    {
        con=connectClass.ConnectDb();
        try{
        
        ps=con.prepareStatement("Select * from Service where (Service_ID || Service_Category || Service_Name || Service_Code || Service_Cost || Description) like '%"+ValueToSearch+"%' order by Service_Category");
        rs=ps.executeQuery();
    ...
    private void fillByName (JTable table, String ValueToSearch)
    {
        con=connectClass.ConnectDb();
        try{
        
        ps=con.prepareStatement("Select * from Service where (Service_ID || Service_Category || Service_Name || Service_Code || Service_Cost || Description) like '%"+ValueToSearch+"%' order by Service_Name");
        rs=ps.executeQuery();
    ...
    private void fillByCost (JTable table, String ValueToSearch)
    {
        con=connectClass.ConnectDb();
        try{
        
        ps=con.prepareStatement("Select * from Service where (Service_ID || Service_Category || Service_Name || Service_Code || Service_Cost || Description) like '%"+ValueToSearch+"%' order by Service_Cost");
        rs=ps.executeQuery();
    ...
    private void initComponents() {
        ...
        combo.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "-Select-", "Service ID", "Service Category", "Service Name", "Service Cost" }));
        ...
        jLabel3.setText("SERVICE REPORT");
    ...
    private void comboActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_comboActionPerformed
        ...
        if(combo.getSelectedItem()=="Service ID")
        {
            d.setText(null);
            d.setEnabled(true);
            DefaultTableModel dm= (DefaultTableModel)table.getModel();
            ...
            dm.setColumnCount(0);
            dm.setRowCount(0);
            dm.addColumn("Service ID");
            dm.addColumn("Service Category");
            dm.addColumn("Service Name");
            dm.addColumn("Service Code");
            dm.addColumn("Service Cost");
            dm.addColumn("Description");
            dm.addColumn("Service Period");
        ...
        if(combo.getSelectedItem()=="Service Category")
        {
            ...
            fillByCategory(table, "");
            
            TableRowSorter<DefaultTableModel> sort=new TableRowSorter<DefaultTableModel>(dm);
            table.setRowSorter(sort);
           
        }
        if(combo.getSelectedItem()=="Service Name")
        {
        ...
        if(combo.getSelectedItem()=="Service Cost")
        {
            d.setText(null);d.setEnabled(true);
            DefaultTableModel dm= (DefaultTableModel)table.getModel();
            dm.setColumnCount(0);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(Service_Report2.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new Service_Report2().setVisible(true);
            }
        });
    }
